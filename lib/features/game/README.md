...
## Structure
- `game/`: Contains the `FlameGame` implementation (`SumBuddyGame`) and all game components (`components/`).
- `presentation/`: Contains the Flutter UI layer that hosts the game.
    - `widgets/`: The `SumBuddyGameWidget` which bridges Flame to Flutter.
...
- **Bridge**: The `SumBuddyGameWidget` connects the Flutter Bloc/Cubit state to the Flame Game via `FlameBlocListeners`.
